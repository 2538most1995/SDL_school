...
9. คะแนนใช้รายวิชาลงทะเบียนจากชุด import ล่าสุดใน district และจำกัดนักศึกษาตามกลุ่มที่ครูรับผิดชอบ; สมุดคะแนนเป็นของขอบเขตอำเภอ/ภาคเรียน/รายวิชา/ระดับ/กลุ่ม ครูที่เข้าถึงกลุ่มเดียวกันจึงบันทึกคะแนนร่วมกันได้โดยไม่ยึดบัญชีผู้สร้าง; การบันทึกของนักศึกษาแต่ละคนเป็นการแทนค่าทั้งชุด ช่องที่เว้นว่างหรือถูกตัดออกและหมายเหตุว่างจะลบ record เดิมเพื่อไม่ให้คะแนนค้าง; ครูสร้างต้นแบบโครงสร้างคะแนนที่ใช้ร่วมกันภายในอำเภอได้ โดยกำหนดให้ใช้ได้กับทุกรายวิชาหรือเฉพาะรหัสวิชาที่อยู่ในขอบเขตของตน ครูทุกคนในอำเภอเลือกใช้ต้นแบบที่ตรงกับรายวิชาปัจจุบันได้ และต้นแบบแบบทุกรายวิชาสามารถสร้างสมุดคะแนนของทุกวิชาที่ครูเข้าถึงได้ในภาคเรียนเดียวด้วยคำสั่งครั้งเดียว โดยข้ามทุกวิชาที่มีสมุดคะแนนอยู่แล้วเพื่อไม่ทับคะแนนเดิม; ครูลบได้เฉพาะต้นแบบที่ตนสร้าง ส่วน admin/super_admin ลบได้ทุกต้นแบบใน district; ครูเลือกโครงสร้างคะแนนเก็บต่อสอบปลายภาคได้เฉพาะ 60:40, 70:30 หรือ 80:20 และนักศึกษาอ่านได้เฉพาะคะแนนของตนเอง สมุดคะแนนเดิมที่ยังไม่มีสัดส่วนยังอ่านได้โดยไม่เดาหรือเปลี่ยนคะแนนย้อนหลัง
...
11. Student login derives district scope from one unique citizen-ID match across current-term rosters in active districts; client-supplied district values are ignored and ambiguous matches fail closed. The login page presents student login first and selects it by default so first-time visitors can enter their citizen ID immediately.
...
20. สถิตินักศึกษาลงทะเบียนนับนักศึกษาไม่ซ้ำที่มีรายการเกรด/ลงทะเบียนในภาคเรียนที่เลือกจาก batch ล่าสุดของอำเภอ แยกและกรองร่วมกันได้ตาม `grp_code`/ชื่อกลุ่ม (กลุ่มเรียน), `occtyp` (กลุ่มเป้าหมาย), `gender`, ระดับตาราง 1–3, `occp` (อาชีพ), `nation` (สัญชาติ) และ `age` (อายุ) พร้อมยอดรวมท้ายตาราง; กลุ่มเรียนที่ชื่อเดียวกันต้องรวมเป็นรายการเดียวข้ามระดับชั้น โดยใช้ชื่อกลุ่มเป็นค่ากรองหลัก; ครูเห็นเฉพาะกลุ่มที่ได้รับมอบหมาย และนักศึกษาไม่มีสิทธิ์เข้ารายงานนี้; การส่งออก Excel ใช้ข้อมูลตามภาคเรียนและตัวกรองเดียวกับหน้าจอ โดย endpoint สำหรับส่งออกอนุญาตเฉพาะ `teacher` และ `admin`
21. ผู้ดูแลอำเภอ (`admin`) สร้างและแก้ไขประกาศป๊อปอัปในขอบเขตอำเภอของบัญชีได้ โดยเปิดใช้งานได้ครั้งละหนึ่งรายการต่ออำเภอ; API อ่านประกาศถูกจำกัดไว้ที่ role `student` และคืนเฉพาะประกาศที่เปิดของอำเภอนักศึกษา ครู ผู้ดูแลอำเภอ และผู้ดูแลสูงสุดจะไม่เห็นป๊อปอัปบนหน้าเว็บของตน ลิงก์ปุ่มยอมรับเฉพาะ `http`/`https` และข้อความแสดงแบบ plain text
22. รายชื่อนักศึกษามีสิทธิ์สอบนับนักศึกษาไม่ซ้ำตามระดับและรหัสจากรายการลงทะเบียนของภาคเรียนที่เลือกใน batch สำเร็จล่าสุด โดยตัดสิทธิ์เฉพาะผู้ที่มีสถานะ `ม` หรือ `มส` อย่างน้อยหนึ่งรายการในภาคเรียนนั้น; คะแนนตัวเลขไม่ใช้ตัดสินสิทธิ์ สถิติรายกลุ่มรวมชื่อกลุ่มเดียวกันข้ามระดับและแสดงจำนวนประถมศึกษา มัธยมศึกษาตอนต้น มัธยมศึกษาตอนปลาย และยอดรวม; ครูเห็นเฉพาะกลุ่มที่ได้รับมอบหมาย ผู้ดูแลเห็นเฉพาะอำเภอที่เลือก และนักศึกษาไม่มีสิทธิ์เข้ารายงานนี้
23. เมนู `สถิติ` รวมเป็นหน้า `รายงานสถิติ` ที่ `/reports/overview` โดยแสดงเฉพาะ 5 รายงานที่มี API และวิธีนับยืนยันแล้ว ได้แก่ นักศึกษาเข้าใหม่ ลงทะเบียน จบการศึกษา คาดว่าจะจบ และนักศึกษาที่มีรายการเทียบโอน; รายงานลงทะเบียนจำแนกได้ 7 มิติจากข้อมูลต้นทาง (`level`, `group`, `gender`, `age`, `occupation`, `target_group`, `nationality`) ส่วนรายงานรายชื่อนักศึกษาใช้ระดับชั้น/กลุ่มเรียน/เพศ และเลือกกรองกลุ่มเรียนก่อนประมวลผลได้; เฉพาะรายงานคาดว่าจะจบเพิ่มมิติและคอลัมน์สถานะ N-Net/E-Exam สองค่า คือ `สอบแล้ว` เมื่อพบภาคเรียนสอบที่ normalize ได้ คะแนนสาระ/วิชาสอบมากกว่า 0 หรือสถานะสอบสำเร็จจากต้นทาง และ `มีสิทธิ์สอบ` เมื่อยังไม่พบประวัติดังกล่าว รายงานอื่นต้องไม่แสดงหรือส่งออกสถานะนี้; รูปแบบแกนตั้งและแกนนอนจัดเก็บแยกตามผู้ใช้ อำเภอ และรายงานใน `statistics_report_preferences` จึงคงค่าเมื่อเปิดใช้ครั้งถัดไป และการเพิ่ม/ลบด้านหนึ่งไม่กระทบอีกด้าน; ตาราง/ไฟล์ส่งออกสรุปยอดผู้เรียนไม่ซ้ำ โดยรายการเทียบโอนหลายวิชาของคนเดียวกันนับเป็นนักศึกษาหนึ่งคนต่อจุดตัด; ทุก query เลือก batch สำเร็จล่าสุดด้วย `district_id` และค้นหาชุดตารางจากชื่อ batch ที่ตรวจสอบแล้ว จึงใช้รูปแบบเดียวกันกับเสนา ไพศาลี และทะเบียนอำเภอที่เพิ่มภายหลัง; หน้านี้ประมวลผลเฉพาะระดับสถานศึกษา/อำเภอและครั้งละหนึ่งภาคเรียนจนกว่าจะมี API รวมข้ามอำเภอ/ช่วงเวลา ห้ามนำข้อมูลรายงานอื่นมาสวมแทนสถานะที่ต้นทางยังแยกไม่ได้
24. การเช็คชื่อเข้าสอบบันทึกแยกตามอำเภอ ภาคเรียน ระดับ และรหัสนักศึกษา โดย roster ใช้นักศึกษาที่มีสิทธิ์สอบจากรายการลงทะเบียนล่าสุด; มุมมองรายวิชาบันทึกแยกวิชา ส่วนมุมมองรายคนแสดงรายชื่อนักศึกษาทั้งหมดและบันทึกสถานะมา/ไม่มาแบบภาพรวมโดยไม่แสดงรายวิชา ครูบันทึกได้เฉพาะกลุ่มที่ได้รับมอบหมาย และผู้ดูแลบันทึกได้เฉพาะอำเภอที่เลือก; นักศึกษาเปิดดูได้เฉพาะรายการรายวิชาและสถานะเข้าสอบของรหัสนักศึกษาที่ผูกกับบัญชีตนเอง โดยไม่มีสิทธิ์แก้ไขหรือติ๊กสถานะ; checkbox ที่ผู้มีสิทธิ์แก้ไขเลือกหมายถึงมาสอบ รายการที่ไม่เลือกหมายถึงขาดสอบ ระบบแสดงจำนวนมาสอบ ขาดสอบ และร้อยละ พร้อมบันทึก audit log ทุกครั้งที่มีการแก้ไข
25. Portal ของ role `student` ใช้ navigation เฉพาะนักศึกษาโดยไม่แสดง sidebar ถาวรบน desktop และมี bottom navigation บนโทรศัพท์ 5 รายการ ได้แก่ หน้าแรก ข้อมูลส่วนตัว ผลการเรียน กพช. และตารางสอบ; หน้าแรกแสดงอัปเดตกิจกรรมเป็นเนื้อหาแรกและไม่แสดงการ์ด hero ภาคเรียน เมนูอื่นยังเข้าถึงได้จาก drawer เมนูทั้งหมด เส้นทางทุกหน้าคงใช้ session, district scope และ student self-scope เดิมโดยไม่มีการขยายสิทธิ์หรือเปลี่ยน API contract
26. เวลาเริ่มและเวลาสิ้นสุดของกิจกรรมปฏิทินเป็นข้อความบังคับที่พิมพ์ระบุได้อิสระไม่เกิน 80 ตัวอักษรในแต่ละวัน; เมื่อทั้งสองค่าเป็นเวลา `HH:mm` ระบบยังตรวจว่าเวลาสิ้นสุดอยู่หลังเวลาเริ่มและใช้เวลาจริงสำหรับเรียงรายการ แต่เมื่อเป็นคำอธิบาย เช่น `ช่วงเช้า` หรือ `หลังเลิกเรียน` ระบบเก็บข้อความไว้ใน `daily_schedule` และใช้ต้นวัน/ท้ายวันเป็น timestamp ภายในเพื่อคงการเรียงตามวันที่โดยไม่เปลี่ยน schema
