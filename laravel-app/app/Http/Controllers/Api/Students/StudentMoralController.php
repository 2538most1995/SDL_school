...
final class StudentMoralController extends StudentsApiController
{
    ...
    public function __invoke(Request $request, string $student): JsonResponse
    {
        ...
        $result = $this->academics->moral($request->user(), $student, $filters['term'] ?? null);
        abort_if($result === null, 404, 'ไม่พบข้อมูลนักศึกษา');

        return response()->json([
            'data' => [
                'student' => (new StudentSummaryResource($result['student']))->resolve($request),
                'items' => array_map(static fn ($assessment): array => (new MoralAssessmentResource($assessment))->resolve($request), $result['items']),
