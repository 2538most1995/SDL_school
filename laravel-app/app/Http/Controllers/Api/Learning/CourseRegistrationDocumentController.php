...
use App\Domain\Students\Services\CourseRegistrationExportService;
...
final class CourseRegistrationDocumentController extends Controller
{
    ...
    public function html(Request $request, CourseRegistrationExportService $export): Response
    {
        $user = $this->resolveUser($request);
        $this->resolveDistrict($request, $user);
        $filters = $this->filters($request);
        $selection = $export->build($user, $filters);
        ...
        return response()->view('print.course-registration-view', $selection, 200, [
            ...$this->privateHeaders(),
            'Content-Type' => 'text/html; charset=UTF-8',
        ]);
    }

    public function pdf(Request $request, CourseRegistrationExportService $export): Response
    {
        $user = $this->resolveUser($request);
        $this->resolveDistrict($request, $user);
        $filters = $this->filters($request);
        ...
        $mpdf->WriteHTML(view('pdf.course-registration', $selection)->render());
    ...
    private function verifySignature(Request $request): bool
    {
        ...
        if ($signature !== '' && $expires >= now()->timestamp) {
            ...
            $expected = hash_hmac('sha256', 'course-registration:'.http_build_query($signedPayload), (string) config('app.key'));
    ...
    {
        $payload = [];
        foreach (['scope', 'student', 'group', 'level', 'term', 'disposition', 'user_id', 'district_id', 'expires', 'openExternalBrowser'] as $key) {
    ...
    private function signatureFor(array $query): string
    {
        return hash_hmac(
            'sha256',
            'course-registration:'.http_build_query($this->signedPayload($query)),
    ...
    private function documentUrl(Request $request, string $document, array $query): string
    {
        ...
        return "{$basePath}/learning/registration/{$document}?".http_build_query($query);
    }
    ...
    /** @return array{scope: string, student?: string, group?: string, level?: int, term?: string} */
    private function filters(Request $request): array
    {
        return $request->validate([
            'scope' => ['required', Rule::in(['student', 'group', 'blank'])],
            'student' => ['nullable', 'string', 'max:64', 'required_if:scope,student'],
            'group' => ['nullable', 'string', 'max:120', 'required_if:scope,group'],
            'level' => ['nullable', 'integer', Rule::in([1, 2, 3]), 'required_if:scope,blank'],
    ...
    {
        $scope = preg_replace('/[^a-z]/', '', (string) $filters['scope']) ?: 'registration';

        return "course-registration-{$scope}-{$count}.pdf";
    }
}
