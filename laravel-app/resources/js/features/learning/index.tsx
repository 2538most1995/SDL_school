...
function ExamSchedulePage() {
    ...
    const signedUrlQuery = useQuery({
        ...
        queryFn: ({ signal }) => {
            ...
            return getFeatureDataWithDemo<{ url: string; pdf_url?: string }>(`/api/v1/learning/exam-schedule/signed-url?${query.toString()}`, { url: '', pdf_url: '' }, signal);
        },
        enabled: canGenerate,
    });
    ...
    return <div>
        ...
        {/* Mobile Action Button: Only "เปิดใน Safari / Chrome" */}
        <div className="mt-5 lg:hidden">
            <Button
                as="a"
                href={directExternalPdfUrl}
                target="_blank"
        ...
        <div className="mt-5 hidden flex-wrap justify-end gap-2 lg:flex">
            ...
            {pdfUrl && <>
                <Button as="a" href={signedUrlQuery.data?.data?.pdf_url || `${directExternalPdfUrl}&disposition=attachment`} download={pdfName} appearance="outline" size="large" icon={<DownloadSimple size={18} weight="bold" />}>ดาวน์โหลด PDF</Button>
                <Button as="a" href={directExternalPdfUrl || pdfUrl} target="_blank" rel="noopener noreferrer" appearance="outline" size="large" icon={<Printer size={18} weight="bold" />}>เปิดเพื่อพิมพ์</Button>
            </>}
        </div>
        </Panel>
