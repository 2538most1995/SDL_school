...
type Workspace = {
    ...
    selected_student: StudentOption | null;
    items: AttendanceItem[];
    summary: { registered_students: number; attended_students: number; absent_students: number; attendance_rate: number };
};

const emptyWorkspace: Workspace = { view: 'subject', terms: [], selected_term: null, subjects: [], students: [], selected_subject: null, selected_student: null, items: [], summary: { registered_students: 0, attended_students: 0, absent_students: 0, attendance_rate: 0 } };
...
export function ExamAttendanceCheckPage() {
    ...
    const params = useMemo(() => {
        ...
        if (student) query.set('student_code', student);
        return query;
    }, [group, level, student, subject, term, view]);
    ...
    useEffect(() => {
        ...
        if (view === 'student' && !student && data.selected_student) {
            setLevel(String(data.selected_student.level));
            setStudent(data.selected_student.student_code);
        }
        setAttendance(Object.fromEntries(data.items.map((item) => [rowKey(item), item.attended])));
    }, [data, student, subject, term, view]);
    ...
    const summaryUnit = view === 'subject' ? 'คน' : 'วิชา';
    ...
    const columns = useMemo<ColumnDef<AttendanceItem>[]>(() => [
        {
            id: view === 'subject' ? 'student' : 'subject',
            header: view === 'subject' ? 'นักศึกษา' : 'รายวิชา',
            accessorFn: (item) => view === 'subject' ? item.full_name : item.subject_name,
            size: 320,
            cell: ({ row }) => view === 'subject'
                ? <div><p className="font-bold text-slate-950">{row.original.full_name}</p><p className="text-xs text-slate-500">{row.original.student_code}</p></div>
                : <div><p className="font-bold text-slate-950">{row.original.subject_name}</p><p className="text-xs text-slate-500">{row.original.subject_code}</p></div>,
        },
        ...
        { accessorKey: 'level_label', header: 'ระดับชั้น', size: 210 },
        {
            ...
            cell: ({ row }) => <label className="inline-flex cursor-pointer items-center gap-2 font-bold text-slate-800"><input type="checkbox" checked={Boolean(attendance[rowKey(row.original)])} onChange={(event) => setAttendance((current) => ({ ...current, [rowKey(row.original)]: event.target.checked }))} className="size-6 rounded border-slate-300 accent-brand-700" aria-label={`เข้าสอบ ${view === 'subject' ? row.original.full_name : row.original.subject_name}`} /><span>{attendance[rowKey(row.original)] ? 'มาสอบ' : 'ขาดสอบ'}</span></label>,
        },
    ], [attendance, view]);
    ...
    return <div>
        <PageHeader category="จัดการการสอบ" title="เช็คชื่อเข้าสอบ" description="บันทึกการเข้าสอบด้วย checkbox ได้ทั้งแบบรายวิชาและรายคน พร้อมสรุปจำนวนและร้อยละทันที" icon={ClipboardText} />
        ...
        <Panel title="เลือกขอบเขตการเช็คชื่อ" description="รายชื่อจำกัดตามอำเภอและกลุ่มที่ผู้ใช้งานรับผิดชอบ">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <label><span className="mb-2 block text-sm font-bold">ภาคเรียน</span><select value={term} onChange={(e) => { setTerm(e.target.value); setSubject(''); setStudent(''); }} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3"><option value="">เลือกภาคเรียน</option>{(data?.terms ?? []).map((item) => <option key={item}>{item}</option>)}</select></label>
                <label><span className="mb-2 block text-sm font-bold">ระดับชั้น</span><select value={level} onChange={(e) => { setLevel(e.target.value); setGroup(''); setSubject(''); setStudent(''); }} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3"><option value="">ทุกระดับ</option><option value="1">ประถมศึกษา</option><option value="2">มัธยมศึกษาตอนต้น</option><option value="3">มัธยมศึกษาตอนปลาย</option></select></label>
                <label><span className="mb-2 block text-sm font-bold">กลุ่มเรียน</span><select value={group} onChange={(e) => { setGroup(e.target.value); setSubject(''); setStudent(''); }} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3"><option value="">ทุกกลุ่ม</option>{groups.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label>
                {view === 'subject' ? <label><span className="mb-2 block text-sm font-bold">รายวิชา</span><select value={subject} onChange={(e) => setSubject(e.target.value)} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3"><option value="">เลือกรายวิชา</option>{(data?.subjects ?? []).map((item) => <option key={`${item.level}|${item.code}`} value={item.code}>{item.code} · {item.name}</option>)}</select></label> : <label><span className="mb-2 block text-sm font-bold">นักศึกษา</span><select value={student} onChange={(e) => setStudent(e.target.value)} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3"><option value="">เลือกนักศึกษา</option>{(data?.students ?? []).map((item) => <option key={`${item.level}|${item.student_code}`} value={item.student_code}>{item.student_code} · {item.full_name}</option>)}</select></label>}
            </div>
        </Panel>
        <StatGrid>
            <StatTile label="ผู้มีสิทธิ์/รายการสอบ" value={`${items.length.toLocaleString('th-TH')} รายการ`} detail={view === 'subject' ? 'นักศึกษาในรายวิชาที่เลือก' : 'รายวิชาของนักศึกษาที่เลือก'} icon={UsersThree} tone="sky" />
            <StatTile label="มาสอบ" value={`${attendedCount.toLocaleString('th-TH')} ${summaryUnit}`} detail={`ร้อยละ ${attendanceRate.toFixed(1)}`} icon={CheckCircle} tone="emerald" />
            <StatTile label="ขาดสอบ" value={`${absentCount.toLocaleString('th-TH')} ${summaryUnit}`} detail={`ร้อยละ ${(items.length ? 100 - attendanceRate : 0).toFixed(1)}`} icon={Prohibit} tone="rose" />
            <StatTile label="อัตราการเข้าสอบ" value={`${attendanceRate.toFixed(1)}%`} detail={`${attendedCount.toLocaleString('th-TH')} จาก ${items.length.toLocaleString('th-TH')} รายการ`} icon={CheckSquare} tone="amber" />
        </StatGrid>
        <Panel title={view === 'subject' ? 'เช็คชื่อตามรายวิชา' : 'เช็คชื่อตามรายคน'} description="ติ๊กถูกเมื่อมาสอบ รายการที่ไม่ติ๊กจะบันทึกเป็นขาดสอบ" action={<div className="flex flex-wrap gap-2"><Button appearance="outline" onClick={() => setAttendance(Object.fromEntries(items.map((item) => [rowKey(item), false])))} disabled={items.length === 0}>ล้างทั้งหมด</Button><Button appearance="outline" onClick={() => setAttendance(Object.fromEntries(items.map((item) => [rowKey(item), true])))} disabled={items.length === 0}>เลือกทั้งหมด</Button><Button appearance="primary" icon={<FloppyDisk size={18} />} onClick={() => save.mutate()} disabled={items.length === 0 || save.isPending}>{save.isPending ? 'กำลังบันทึก' : 'บันทึกการเข้าสอบ'}</Button></div>}>
            ...
            {data && <DataTable data={items} columns={columns} pageSize={50} responsiveMode="cards" disableExport emptyTitle="ไม่พบรายชื่อสำหรับเช็คชื่อ" emptyDescription="ลองเปลี่ยนภาคเรียน ระดับ กลุ่ม รายวิชา หรือนักศึกษา" />}
        </Panel>
    </div>;
}
