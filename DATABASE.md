...
- `learning_assignments`: district, creator, academic term, subject code/name, education level, title/instructions, target type/value, score, open/due timestamps, status, optional teacher material URL and private PDF/image metadata; index (`district_id`, `status`, `due_at`)
- `learning_submissions`: assignment, imported `student_code` snapshot, optional canonical `student_id`, link/PDF/image metadata, private attachment compatibility columns, submission/review status, score/feedback; UNIQUE (`assignment_id`, `student_code`)
- `learning_submission_attachments`: private PDF/image files belonging to one submission, with server-owned path, MIME, size and display order; UNIQUE (`submission_id`, `position`). Student image submission supports up to 10 files while the original single-file columns remain for backward compatibility.
...
- `exam_rooms` lookup uses district + term and matches exact/wildcard subject plus group/student ranges. หน้า admin จำกัดรายการและการเขียนไว้ที่ภาคเรียนปัจจุบันจาก roster ล่าสุด พร้อมคำนวณขอบเขตตำบล/ระดับชั้นจากช่วงเดิมโดยไม่เพิ่มคอลัมน์ซ้ำในตาราง เมื่อภาคเรียนปัจจุบันว่าง การ carry forward จะคัดลอกแถวของภาคเรียนก่อนหน้าล่าสุดภายใน transaction เดียว เปลี่ยน `term` เป็นปัจจุบัน รักษาแถวต้นฉบับ และล็อก district ก่อนตรวจซ้ำ; คำสั่ง insert ละ `import_batch_id` ไว้เพื่อรองรับฐาน production รุ่นเดิมที่ยังไม่มีคอลัมน์นี้ (ฐานรุ่นใหม่จะใช้ค่า `null` โดยปริยาย)
