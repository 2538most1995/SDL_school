...
The canonical student and learning migrations inspect the actual type of `students.id` before creating `student_id`. This supports an adopted local `students` table whose primary key is signed `INT`, repairs child tables left behind by a failed MySQL foreign-key statement, and never deletes orphaned rows to force a constraint.
