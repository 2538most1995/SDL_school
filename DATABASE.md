...
- `learning_assignments`: district, creator, academic term, subject code/name, education level, title/instructions, target type/value, score, open/due timestamps, status, optional teacher material URL and private PDF metadata; index (`district_id`, `status`, `due_at`)
- `learning_submissions`: assignment, imported `student_code` snapshot, optional canonical `student_id`, link/PDF metadata, private attachment, submission/review status, score/feedback; UNIQUE (`assignment_id`, `student_code`)
